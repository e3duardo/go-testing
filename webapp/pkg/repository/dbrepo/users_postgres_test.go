...
func Test_pingDB(t *testing.T) {
	err := testDB.Ping()
	if err != nil {
		t.Errorf("can't ping the database")
	}
}
...
func TestPostgresDBRepoGetUser(t *testing.T) {
	...
	if err == nil {
		t.Errorf("non error reported when getting non existent user by id")
	}
}
...
func TestPostgresDBRepoInsertImage(t *testing.T) {
	...
	if newId != 1 {
		t.Errorf("got wrong id for image, should 1, but got %d", newId)
	}

	image.UserID = 100
	_, err = testRepo.InsertUserImage(image)
	if err == nil {
		t.Errorf("inserted a user image with non-existent user id: %s", err)
	}
}
